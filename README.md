...
The agent follows a simple but powerful loop: Observe → Decide → Act → Reflect → Stop
...
Homepage (listing) → Agent detects products → Opens a product page → Observes lack of interactive CTAs → Returns to listing → Detects repeated state → Stops intentionally
...
num_products: 20
has_next_page: True
page_type: listing Decision: open_product Action: Open first product
--- Step 2 ---
...
num_products: 0
has_next_page: False
page_type: product Decision: go_back Action: Go back to listing
--- Step 3 --- Repeated page detected. Stopping.
